...
RSpec.shared_examples 'deployment with valid data' do
  it 'assigns a new deployment instance as @deployment' do
    post :validate, { deployment: valid_attributes, template_id: template.to_param, provider: provider_object.textkey, format: :json }, valid_session
...
RSpec.shared_examples 'deployment with invalid data' do |use_invalid_attributes|
  ...
  it 'validate responds with error' do
    post :validate, { deployment: use_invalid_attributes ? invalid_attributes : valid_attributes, template_id: template.to_param, provider: provider_object.textkey, format: :json }, valid_session
...
RSpec.shared_examples 'deployment options with valid data' do |type|
  it 'assigns a new deployment instance as @deployment' do
    post :options, { deployment: valid_attributes, template_id: template.to_param, type: type, provider: provider_object.textkey, format: :json }, valid_session
    expect(assigns(:decorator)).to be_a(DeploymentDecorator)
    expect(assigns(:decorator).deployment).to be_a_new(Deployment)
    expect(assigns(:decorator).deployment).not_to be_persisted
  end
...
RSpec.shared_examples 'deployment options with invalid data' do |type, use_invalid_attributes|
  ...
  it 'validate responds with error' do
    post :options, { deployment: use_invalid_attributes ? invalid_attributes : valid_attributes, template_id: template.to_param, type: type, provider: provider_object.textkey, format: :json }, valid_session
    expect(response).to have_http_status(:unprocessable_entity)
  end
end
