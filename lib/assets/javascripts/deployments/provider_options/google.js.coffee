...
class SiteFull.Deployments.ProviderOptions.Google extends SiteFull.Deployments.ProviderOptions.Base
  init: ->
    requests = []
    ...
    ).fail(@disable_instance_inputs).always( -> $('body').removeClass('loading') )
